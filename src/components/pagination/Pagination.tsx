...
const paginationRange = (totalPages: number, page: number, limit: number, siblings: number) => {
  let result: PageNavigationSymbols[] = [];
  let totalPageInArray = siblings;

  if (totalPageInArray > totalPages) result = _.range(1, totalPages + 1);
...
export function Pagination(props: PaginationProps) {
  const { totalPages, page, limit, siblings, onPageChange } = props;
  const pageArray = paginationRange(totalPages, page, limit, siblings);
